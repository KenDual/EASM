...
import { Drawer, Toast, icon, statusBadge, typeBadge, fmtRelative, fmtDateTime, fmtDuration, escapeHtml, copyToClipboard } from './ui.js';
import { getAsset, trackJob, loadAssets, activeJobsForAsset, subscribe, refreshAsset } from './state.js';

// Scanner catalog with applicability + display metadata.
const SCANNERS = [
  { type: 'dns',       label: 'DNS',         appliesTo: ['domain'], desc: 'A/AAAA/MX/NS/TXT/CNAME records' },
  { type: 'whois',     label: 'WHOIS',       appliesTo: ['domain'], desc: 'Registrar + creation/expiry' },
  { type: 'subdomain', label: 'Subdomains',  appliesTo: ['domain'], desc: 'crt.sh + bruteforce list' },
  { type: 'cert_trans',label: 'CT Logs',     appliesTo: ['domain'], desc: 'Certificate transparency log entries' },
  { type: 'ssl',       label: 'SSL/TLS',     appliesTo: ['domain'], desc: 'Certificate chain + expiry' },
  { type: 'tech',      label: 'Tech',        appliesTo: ['domain'], desc: 'HTTP headers + tech fingerprint' },
  { type: 'asn',       label: 'ASN',         appliesTo: ['ip'],     desc: 'BGP/ASN ownership' },
  { type: 'ip',        label: 'IP info',     appliesTo: ['ip'],     desc: 'Geo + reverse DNS' },
  { type: 'port',      label: 'Ports',       appliesTo: ['ip'],     desc: 'Common port sweep (private only)' },
];
...
export async function openAssetDrawer(assetId, opts = {}) {
  ...
  if (!asset) {
    try { asset = await api.getAsset(assetId); } catch (err) { Toast.error(err.message); return; }
  }
...
function wireShellEvents(el, asset, scanners) {
  el.querySelector('[data-drawer-close]').addEventListener('click', () => Drawer.close());
  el.querySelector('[data-copy]').addEventListener('click', (e) => copyToClipboard(e.currentTarget.dataset.copy));
...
function renderTabBar(root, asset, scanners) {
  ...
  const tabDefs = [
    ...
    ...scanners.map((s) => ({ id: s.type, label: s.label, hasData: !!cachedResults[s.type] })),
...
function renderOverviewTab(el, asset) {
  ...
  const scannersForType = SCANNERS.filter((s) => s.appliesTo.includes(asset.type));
...
function renderScannerTab(el, asset, scanType) {
  ...
  el.innerHTML = `
    <div class="space-y-4">
      ...
      ${result ? renderResultBody(scanType, result) : `
...
function renderPretty(scanType, data) {
            ...
            <tr>
              ...
              <td>${
                Array.isArray(v) && v.length
                  ? v.map((x) => `<span class="badge badge-neutral mono">${escapeHtml(typeof x === 'object' ? JSON.stringify(x) : x)}</span>`).join(' ')
  ...
  if (scanType === 'subdomain') {
    const subs = Array.isArray(first?.subdomains) ? first.subdomains : (Array.isArray(first) ? first : []);
    return `
  ...
  if (scanType === 'port') {
    const ports = Array.isArray(first?.open_ports) ? first.open_ports : (Array.isArray(first) ? first : []);
  ...
  if (scanType === 'whois') {
    return `
      <div class="panel"><div class="panel-header"><span class="panel-title">WHOIS</span></div>
        <div class="p-4"><dl class="kv">
          ${Object.entries(first).slice(0, 14).map(([k, v]) => kvRow(k, formatScalar(v))).join('')}
        </dl></div>
      </div>`;
  }

  if (scanType === 'tech' || scanType === 'ip' || scanType === 'asn' || scanType === 'cert_trans') {
    return `
      <div class="panel"><div class="panel-header"><span class="panel-title">${escapeHtml(scanType)}</span></div>
        <div class="p-4"><dl class="kv">
          ${Object.entries(first).slice(0, 20).map(([k, v]) => kvRow(k, formatScalar(v))).join('')}
        </dl></div>
      </div>`;
  }
