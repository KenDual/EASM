...
export async function renderOverview(container) {
  ...
  `;
  container.querySelector('#overview-refresh').addEventListener('click', () => load(true));
  await load();
}
...
function renderKPIs(el, assets, jobs) {
  ...
  const failed24h = jobs.filter((j) => j.status === 'failed' && (Date.now() - new Date(j.created_at).getTime()) < 86400000).length;

  el.innerHTML = [
    kpi('Total assets', assets.length, 'fg', `${domains} domains · ${ips} IPs`),
    kpi('Scan jobs', jobs.length, 'fg', `${jobs.filter(j => j.status === 'completed').length} completed`),
    kpi('Active now', active, active > 0 ? 'info' : 'muted', active > 0 ? 'scanning…' : 'idle'),
    kpi('Failures (24h)', failed24h, failed24h > 0 ? 'danger' : 'success', failed24h === 0 ? 'all good' : 'check jobs'),
  ].join('');
}

function kpi(label, value, color = 'fg', sub = '') {
  const colorClass = {
    fg: 'text-fg', success: 'text-success', warning: 'text-warning', danger: 'text-danger', info: 'text-info', muted: 'text-fg-muted'
  }[color] || 'text-fg';
  return `
    <div class="panel p-3">
...
function renderRecent(el, list) {
  ...
  if (loading && list.length === 0) {
    el.innerHTML = `<div class="p-4 space-y-2">${Array.from({ length: 5 }).map(() => '<div class="skeleton h-8 w-full"></div>').join('')}</div>`;
    return;
  }
  if (!list.length) {
