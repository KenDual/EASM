...
import { Drawer, Modal, Toast, icon, typeBadge, statusBadge, fmtRelative, escapeHtml, copyToClipboard } from './ui.js';
import { getState, loadAssets, subscribe, activeJobsForAsset } from './state.js';
...
export async function renderAssets(container) {
  ...
  container.querySelector('#refresh-assets').addEventListener('click', async () => {
    await loadAssets({ force: true });
  });
  container.querySelector('#open-new-asset').addEventListener('click', openCreateAssetModal);
  ...
  container.querySelectorAll('.type-filter').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      container.querySelectorAll('.type-filter').forEach((b) => {
        b.classList.toggle('btn-secondary', b.dataset.type === filter.type);
...
function getFilteredAssets() {
  ...
  return assets.filter((a) => {
    ...
    if (filter.q && !a.name.toLowerCase().includes(filter.q.toLowerCase())) return false;
    return true;
  });
}
...
function renderTable() {
  ...
  countEl.textContent = assetsLoaded ? `${list.length} ${list.length === 1 ? 'asset' : 'assets'}` : '';

  if (!assetsLoaded && assetsLoading) {
    wrap.innerHTML = `<div class="p-4 space-y-2">
      ${Array.from({ length: 5 }).map(() => '<div class="skeleton h-9 w-full"></div>').join('')}
    </div>`;
    return;
  }
